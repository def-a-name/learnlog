...
// 将 fmt_string_view 类型的字符串形式加入 dest_buf
...
template <typename T>
void append_int(T n, fmt_memory_buf& dest_buf) {
    fmt::format_int i(n);
...
// 将 n 转换为无符号整型，再计算位数
template <typename T>
size_t count_unsigned_digits(T n) {
    using T_type =
...
template <typename T>
void fill_uint_(T n, size_t n_len, size_t fill_len, fmt_memory_buf& dest_buf) {
...
template <typename T>
void fill_uint(T n, size_t fill_len, fmt_memory_buf& dest_buf) {
...
Metric precise_time(sys_clock::time_point tp) {
